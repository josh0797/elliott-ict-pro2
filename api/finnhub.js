...
export default async function handler(req, res) {
  try {
    ...
    // Mapas limpios sin espacios erróneos
    const FX_MAP = {
      ...
      EURGBP: "OANDA:EUR_GBP", XAUUSD: "OANDA:XAU_USD",
    };
    ...
    if (type === "stock") {
      ...
      symbol = FX_MAP[ticker.toUpperCase()];
    } else if (type === "crypto") {
      endpoint = "crypto/candle";
      symbol = CRYPTO_MAP[ticker.toUpperCase()];
    }

    if (!symbol) {
      return res.status(400).json({
        ok: false,
        error: `Finnhub: No symbol mapping for type=${type} ticker=${ticker}`
      });
    }

    const to = nowUnix();
    // Lookback optimizado para análisis técnico
